...
export default function CFBPage() {
    ...
    const fetchData = async () => {
        try {
            const [scheduleRes, signalsRes] = await Promise.all([
                fetch('/api/engine/proxy?endpoint=api/data/schedule&sport=cfb'),
                fetch('/api/engine/proxy?endpoint=api/signals/cfb')
            ]);
            ...
            console.error("CFB dashboard sync failed:", e);
        } finally {
            setLoading(false);
        }
    };
    ...
    if (loading) return <div className={styles.loading}>ESTABLISHING FIELD DATA...</div>;
            ...
            <div className={styles.container}>
                ...
                <div className={styles.header}>
                    <h1 className={styles.title}>
                        <span style={{ fontSize: '2.5rem' }}>🏈</span> CFB HUB
                    </h1>
                ...
                <h2>LIVE SLATE</h2>
                {schedule.length === 0 ? (
                    <div className={styles.noGames}>NO CFB GAMES SCHEDULED FOR TODAY. (OFF-SEASON)</div>
                ) : (
